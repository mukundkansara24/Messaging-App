...
class AppWriteService {
  ...
  async listSender({ userId }) {
    try {
      ...
      response.rows.map((row) => {
        if (row.SenderID !== userId) uniqueUsers.add(row.SenderID);
        if (row.ReceiverID !== userId) uniqueUsers.add(row.ReceiverID);
      });
      return [...uniqueUsers];
    } catch (err) {
  ...
  async getUsernameFromUserId({ userId }) {
    try {
      const response = await this.tablesDB.listRows({
        ...
        queries: [Query.select(["Username"]), Query.equal("$id", userId)],
      });
      return response;
    } catch (err) {
  ...
  async searchByUsername({ userName }) {
    try {
      const response = await this.tablesDB.listRows({
        databaseId: conf.appwriteDatabaseId,
        tableId: conf.appwriteUserId,
        queries: [Query.equal("Username", userName), Query.select(["$id"])],
      });
      return response;
    } catch (err) {
  ...
  subscribeToMessages(userId, onMessageReceived) {
    ...
    const unsubscribe = this.client.subscribe(channel, (response) => {
      ...
      let isCreate = false;
      for (const value of Object.values(response.events)) {
        // or use .forEach()
      ...
      if (
        isCreate &&
        (response.payload.SenderID === userId ||
          response.payload.ReceiverID === userId)
      ) {
